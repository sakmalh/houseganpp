...
from misc.utils import draw_masks
import torch
...
parser.add_argument("--n_critic", type=int, default=10000, help="number of training steps for discriminator per iter")
parser.add_argument("--target_set", type=int, default=8, choices=[5, 6, 7, 8], help="which split to remove")
parser.add_argument("--data_path", type=str, default="/home/akmal/APIIT/FYP Code/Housegan-data-reader/result2.txt",
...
def visualizeSingleBatch(fp_loader_test, opt, exp_folder, batches_done, batch_size=8):
    ...
    with torch.no_grad():
        ...
        gen_mks_real = generator(z, given_masks_in, given_nds, given_eds)
        # Generate a batch of images
        gen_mks_fake = generatorTest(z, given_masks_in, given_nds, given_eds)
        # Generate image tensors
        gen_mks_real = gen_mks_real.detach().cpu().numpy()
        gen_mks_fake = gen_mks_fake.detach().cpu().numpy()

        real_imgs_tensor = draw_masks(gen_mks_real.copy(), real_nodes)
        real_imgs_tensor = torch.tensor(np.array(real_imgs_tensor).transpose((2, 0, 1))) / 255.0

        fake_imgs_tensor = draw_masks(gen_mks_fake.copy(), real_nodes)
        fake_imgs_tensor = torch.tensor(np.array(fake_imgs_tensor).transpose((2, 0, 1))) / 255.0
        # real_imgs_tensor = combine_images(real_mks, given_nds, given_eds, nd_to_sample, ed_to_sample)
        # fake_imgs_tensor = combine_images(gen_mks, given_nds, given_eds, nd_to_sample, ed_to_sample)
        # Save images
        save_image(real_imgs_tensor, "./exps/{}/{}_real.png".format(exp_folder, batches_done), nrow=1, normalize=False)
        save_image(fake_imgs_tensor, "./exps/{}/{}_fake.png".format(exp_folder, batches_done), nrow=1, normalize=False)
    return
...
fp_loader = torch.utils.data.DataLoader(fp_dataset_train,
                                        ...
                                        pin_memory=False)
fp_dataset_test = FloorplanGraphDataset(opt.data_path, transforms.Normalize(mean=[0.5], std=[0.5]),
